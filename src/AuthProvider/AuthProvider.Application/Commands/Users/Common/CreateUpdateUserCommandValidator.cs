...
public class CreateUpdateUserCommandValidator<TCommand> : AbstractValidator<TCommand>
	where TCommand : ICreateUpdateUserCommand
{
		...
		When(pet => pet.States != null, () =>
		{
			...
			RuleFor(x => x.States)
				...
				.WithMessage(p =>
					...
					"are conflicted. Pet can have only one listed status at a time")
				.Must(x =>
				{
